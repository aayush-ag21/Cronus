# This code is for whole system, all motors and all servos
import threading
import time
import os
try:
...
def height(h, tx, ty) :
      ...
      t3.start()
      t4=threading.Thread(target=servo,args=(RBL,75-min(max(15,(h+tx-ty)),75)))
      t4.start()
      t5=threading.Thread(target=servo,args=(LFU,105-min(max(15,(h-tx+ty)),75)))
      t5.start()
      t6=threading.Thread(target=servo,args=(LFL,75-min(max(15,(h-tx+ty)),75)))
      t6.start()
...
def motor(pin1, pin2, pin3, s):     #front pin1, back pin2 , PWM pin3
      ...
      if (s>0):
        ...
        pi.write(pin1,0)
        pi.write(pin2,0)
      pi.set_PWM_dutycycle(pin3,min(100,max(s,0)))
